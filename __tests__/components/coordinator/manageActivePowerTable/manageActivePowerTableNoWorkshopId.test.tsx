...
import { updateActiveVoterNoWorkshopIdHandler } from '@/../__mocks__/updateActiveVoter/errorHandlers';
...
test('Successfully alerts when workshop id is not provided', async () => {
  server.use(...updateActiveVoterNoWorkshopIdHandler);
  ...
  const errorToast = await screen.findByText('Workshop id must be provided.');
