...
import { updateUserNotOrganizerHandler } from '@/../__mocks__/updateUser/errorHandlers';
...
test('Successfully alerts on internal error', async () => {
  server.use(...updateUserNotOrganizerHandler);
  ...
  const errorToast = await screen.findByText(
    'You must be an Organizer to update user information.',
  );
