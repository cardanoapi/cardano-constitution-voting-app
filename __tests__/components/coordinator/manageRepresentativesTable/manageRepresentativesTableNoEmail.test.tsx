...
import { updateUserNoEmailHandler } from '@/../__mocks__/updateUser/errorHandlers';
...
test('Successfully alerts on internal error', async () => {
  server.use(...updateUserNoEmailHandler);
  ...
  const errorToast = await screen.findByText('Email must be provided.');
