...
import { updateUserTooLongWalletAddressHandler } from '@/../__mocks__/updateUser/errorHandlers';
...
test('Successfully alerts on internal error', async () => {
  server.use(...updateUserTooLongWalletAddressHandler);
  ...
  const errorToast = await screen.findByText(
    'Wallet Address must be less than 100 characters.',
  );
