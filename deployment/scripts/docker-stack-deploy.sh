...
function docker-stack-deploy(){
      ...
      if [[ $WITH_REGISTRY_AUTH == "--with-registry-auth" ]]
      then
        docker stack deploy $WITH_REGISTRY_AUTH -c "$RENDERED_FILENAME" ${STACK_NAME}
      else
